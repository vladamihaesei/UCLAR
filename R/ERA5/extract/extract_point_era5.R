...
files <- grep("10m_v_component|lcc|tcc",files,invert = T, value = T)## elimina vantul 
...
###### extract for radiation/cloudcover
###### listeaza fisierele raster
files <- list.files(paste0(drive_d,"dailymerged"),pattern = ".nc", recursive = T,  full.names = T)
files <- grep("radiation|lcc|tcc",files,invert = F, value = T)## elimina vantul 
files.split <- do.call(rbind, strsplit(files, "\\/|_"))
params <- c("/radiation_daily_", "/lcc_dailyresampled_","/tcc_dailyresampled_")
