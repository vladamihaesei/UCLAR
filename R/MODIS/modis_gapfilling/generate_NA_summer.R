...
for (i in 1:length(orase)){
  ...
  for(j in 1:length(types)){
    ...
    for (d in 1:length(dn)){
      ...
      tf <- tabs%>%filter(year(timp) >= 2019 & year(timp) <= 2021 &month(timp) %in% c("5","6","7","8","9") & frecventa <= 0.1)
      tf <- tf[c(round(runif(12, min=1, max=nrow(tf)))),]
      timp <- as.character(tf$timp)
      
      write.csv(tf,paste0("tabs/MODIS/",orase[i],"/",types[j],"/",dn[d],"_","nocloud_days_summer.csv"), row.names = F) 
      tabs1 <- read.csv(paste0("tabs/MODIS/",orase[i],"/","MYD11A1","/","Day","_","cloud_pixels.csv")) 
      t.mask <- tabs1%>%filter(year(timp) >= 2019 & year(timp) <= 2021 &month(timp) %in% c("5","6","7","8","9")& frecventa>= 19&frecventa <= 23)
      t.mask <- t.mask[2,]
      timp1 <- t.mask$timp
